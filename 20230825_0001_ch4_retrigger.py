...
ljh = ljhs[3] #RPF only ch 2, so [0] is ch 2
ljh.set_output_npre_npost(500,500)
ljh.plot_median_value_vs_time(chunk_skip_size=100)
...
my_irange = ljh.nPulses//100 # used in filter and in calculated real time of analysis
my_imin = 10*ljh.nPulses//100
my_imax = my_imin+my_irange
pulse_inds_filter = ljh.fasttrig_filter(imax=my_imax, filter=filter, 
                                        threshold=8000,
                                        imin=my_imin) # imax=ljh.nPulses//10
...
dt_s = my_irange*ljh.nSamples*ljh.timebase
