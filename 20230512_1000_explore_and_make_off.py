...
pulse_files = file_pattern("1000")
noise_files = file_pattern("2000")
...
ds.calibration["p_filt_value"].add_cal_point(1446, 5.486e6)
...
with h5py.File(model_hdf5,"w") as h5:
    mass.make_projectors(pulse_files=pulse_files,
        ...
        mass_hdf5_path=ds.hdf5_group.file.filename+"_alt",
        mass_hdf5_noise_path=ds.noise_records.hdf5_group.file.filename+"_alt",
        invert_data=False,
...
print(f"count rate calculation = ({counts} counts)/({fulltime} s) = {counts/fulltime}")
