...
def file_pattern(runnum):
 ...
 p = os.path.join(f"20230824",f"{runnum}",f"20230824_run{runnum}_chan1.ljh")
 return p
...
ds.linefit(5.486e6, dlo=2e5, dhi=2e5, binsize=2e3) #1e4
...
dsoff = dataoff[2]
dsoff.cutAdd("cutResidualStdDev", lambda residualStdDev: residualStdDev < 20, setDefault=False, overwrite=True)
...
# HISOGRAMS OF None, Cuts, Cuts & energyDC
dsoff.plotHist(np.arange(5.2,5.6,0.001)*1e6, "energy",cutRecipeName=None)
plt.legend("energy, none")
plt.grid(True)
dsoff.plotHist(np.arange(5.2,5.6,0.001)*1e6, "energy", cutRecipeName="cutResidualStdDev")
plt.legend("energy, cut")
plt.grid(True)

dsoff.plotHist(np.arange(5.2,5.6,0.001)*1e6, "energyDC", cutRecipeName="cutResidualStdDev")
plt.legend("energyDC, cut")
plt.grid(True)
dsoff.linefit(lineNameOrEnergy=5.488e6, attr="energyDC",dlo=1.e4, dhi=1.2e4,cutRecipeName="cutResidualStdDev")
# END HISTOGRAMS
