...
ljh = ljhs[1] #RPF only ch 2, so [0] is ch 2
...
filter = np.array([-1]*10+[+1]*10) # negate for negative pulses

my_imax = ljh.nPulses//100 # used in filter and in calculated real time of analysis

pulse_inds_filter = ljh.fasttrig_filter(imax=my_imax, filter=filter, threshold=10000) # imax=ljh.nPulses//10
...
ljh.plot_first_n_samples_with_inds(4000000, pulse_inds=pulse_inds_filter, noise_inds=noise_inds, filter=filter)
